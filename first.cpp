...
/// 2. 将 Python 传入 QP 图转换为“帧基准 QP + 宏块偏移”
/// 3. 逐帧编码并输出 H.264 比特流
///
/// QP 使用策略：
/// - 按 16x16 宏块划分视频帧
/// - 基线模式：强制整帧 CQP，不使用 QP 图
/// - 自适应模式：根据 QP 图计算帧基准 QP，并写入宏块偏移
class X264Encoder
{
public:
	/// 析构函数：自动关闭编码器并释放资源
	...
	/// 5. 禁用 B 帧（简化宏块控制）
	/// 6. 使用恒定质量模式 (CQP)
	bool open(std::uint32_t width, std::uint32_t height, std::ostream& output, int forceCqp = -1)
	{
		close();
		forceCqp_ = forceCqp;
		...
		if (forceCqp_ >= 0) {
			// 基线模式：禁用 AQ，保持严格恒定 QP。
			param_.rc.i_rc_method = X264_RC_CQP;
			param_.rc.i_qp_constant = std::clamp(forceCqp_, 0, 51);
			param_.rc.i_aq_mode = 0;
			param_.rc.f_aq_strength = 0.0F;
		} else {
			// 自适应模式：使用 CRF + AQ，使 quant_offsets 参与率控。
			param_.rc.i_rc_method = X264_RC_CRF;
			param_.rc.f_rf_constant = 23.0F;
			param_.rc.i_aq_mode = 1;
			param_.rc.f_aq_strength = 1.0F;
		}
	...
	/// 2. 调用 buildMacroblockQpMapDirect 将目标 QP 图直接转换为宏块量化数组
	/// 3. 将该数组写入 x264 的宏块量化数组
	/// 4. 调用 x264 编码器进行编码
	/// 5. 将输出的 NAL 单元写入输出流
	bool encode(const FramePacket& packet)
	{
		...
		if (forceCqp_ >= 0) {
			// 基线强制 CQP：完全忽略传入 QP 图，保证 QP 点可分离。
			picture_.i_qpplus1 = std::clamp(forceCqp_, 0, 51) + 1;
			picture_.prop.quant_offsets = nullptr;
			picture_.prop.quant_offsets_free = nullptr;
		} else {
			// 自适应路径：将目标 QP 图转换为“固定参考 QP + 宏块偏移”。
			// 这样当外部对整图做统一 QP 平移时，码率会稳定地发生变化，
			// 不会被“帧均值基准+偏移”分解抵消。
			buildMacroblockQpOffsets(packet);
			// CRF 路径下由率控决定帧级 QP，宏块偏移由 quant_offsets 提供。
			picture_.i_qpplus1 = 0;
			picture_.prop.quant_offsets = mbQpOffsets_.empty() ? nullptr : mbQpOffsets_.data();
			picture_.prop.quant_offsets_free = nullptr;
		}
...
private:
	/// 将 QP 图转换为“固定参考 QP + 宏块偏移”
	/// 
	/// 映射策略：
	/// 1. 计算帧的宏块网格大小（基于 16x16）
	/// 2. 对每个宏块，将其映射到 QP 图坐标（支持任意分辨率）
	/// 3. 取出对应位置的目标 QP 值，约束到 [0, 51]
	/// 4. 使用固定参考 QP（默认 23）作为基准，每个宏块写入 (target-ref) 偏移
	///
	/// 说明：
	/// 若按“每帧均值 + 偏移”分解，整图统一平移 delta 时，偏移会抵消 delta，
	/// 可能导致码率对齐实验不敏感。固定参考基准可避免该问题。
	///
	/// @param packet  输入的帧数据包
	void buildMacroblockQpOffsets(const FramePacket& packet)
	{
		// 计算宏块网格大小（向上取整）
		...
		const float refQp = 23.0F;

		// 如果 QP 图为空或尺寸无效，所有宏块默认使用参考 QP。
		if (packet.qp.empty() || packet.qpWidth == 0 || packet.qpHeight == 0 || mbCount == 0) {
			frameBaseQp_ = static_cast<int>(refQp);
			mbQpOffsets_.assign(mbCount, 0.0F);
			return;
		}

		std::vector<float> mbTargetQp(mbCount, 23.0F);
		double sumQp = 0.0;
		...
		for (std::uint32_t my = 0; my < mbHeight; ++my) {
			for (std::uint32_t mx = 0; mx < mbWidth; ++mx) {
				...
				// 读取对应位置的目标 QP 值（越界时默认为 23）
				const std::uint8_t q = qIndex < packet.qp.size() ? packet.qp[qIndex] : static_cast<std::uint8_t>(23);
				const float qf = static_cast<float>(std::clamp<int>(q, 0, 51));
				mbTargetQp[mbIndex] = qf;
				sumQp += qf;
			}
		}

		// 保留统计字段，便于日志排查；编码时不再作为分解基准。
		frameBaseQp_ = std::clamp<int>(static_cast<int>(std::lround(sumQp / static_cast<double>(mbCount))), 0, 51);

		mbQpOffsets_.assign(mbCount, 0.0F);
		for (std::size_t i = 0; i < mbCount; ++i) {
			mbQpOffsets_[i] = mbTargetQp[i] - refQp;
		}
	}
	...
	std::vector<float> mbQpOffsets_;       ///< 每个宏块相对帧基准 QP 的偏移
	int frameBaseQp_ = 23;                 ///< 当前帧基准 QP（0~51）
	int forceCqp_ = -1;                    ///< >=0 时启用强制 CQP 并忽略传入 QP 图
...
public:
	/// 初始化编码器（降级版：仅记录信息）
	bool open(std::uint32_t, std::uint32_t, std::ostream& output, int = -1)
	{
		output_ = &output;
		return true;
	}
...
///  --force-cqp <0..51> - 强制恒定 QP（基线模式，忽略传入 QP 图）
///
/// 处理流程：
/// 1. 打开输入流（文件或 stdin）
/// 2. 打开输出流
...
int main(int argc, char* argv[])
{
	try {
		...
		// 先解析命令行：支持位置参数 input/output 和可选参数 --force-cqp。
		...
		for (int i = 1; i < argc; ++i) {
			...
			if (arg == "--force-cqp") {
				if (i + 1 >= argc) {
					std::cerr << "missing value for --force-cqp\n";
					return 1;
				}
				forceCqp = std::clamp(std::stoi(argv[++i]), 0, 51);
				continue;
			}
		...
		if (!encoder.open(packet.width, packet.height, outputFile, forceCqp)) {
